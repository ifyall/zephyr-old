...
#define BT_STABILIZATION_DELAY            (250u)
...
uint32_t bt_hci_uart_get_baudrate(const struct device *bt_uart_dev)
{
	struct uart_config uart_cfg;
	uint32_t baudrate = 0;

	/* Get initial UART configuration */
	if (!uart_config_get(bt_uart_dev, &uart_cfg)) {
...
int bt_hci_uart_set_baudrate(const struct device *bt_uart_dev, uint32_t baudrate)
{
	struct uart_config uart_cfg;
	int err = 0;
...
int bt_update_controller_baudrate(const struct device *bt_uart_dev, uint32_t baudrate)
{
	/*
	...
	memcpy(data_buf, hci_data, HCI_VSC_UPDATE_BAUD_RATE_LENGTH);
...
static int bt_firmware_download(const uint8_t *firmware_image, uint32_t size)
{
	...
	while (remaining_length) {
		...
		memcpy(data_buf, &data[3], data_length);
...
int bt_h4_vnd_setup(const struct device *bt_uart_dev)
{
	...
	/* BT settling time after power on */
	k_msleep(BT_POWER_ON_SETTLING_TIME);

	/* Set default baudrate */
	err = bt_hci_uart_set_baudrate(bt_uart_dev, HCI_UART_DEFAULT_BAUDRATE);
	if (err) {
		return err;
	}
	...
	if (baudrate_for_fw_download != HCI_UART_DEFAULT_BAUDRATE) {
		err = bt_update_controller_baudrate(bt_uart_dev, baudrate_for_fw_download);
		if (err) {
			return err;
		}
	}
	...
	/* Stabilization delay */
	k_msleep(BT_STABILIZATION_DELAY
		 );
	...
	if (baudrate_for_operation != HCI_UART_DEFAULT_BAUDRATE) {
		err = bt_update_controller_baudrate(bt_uart_dev, baudrate_for_operation);
		if (err) {
			return err;
		}
	}
