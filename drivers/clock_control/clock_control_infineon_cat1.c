 ...
 */
static struct infineon_cat1_clock_info_t clock_info_table[] = {
...
uint32_t clock_info_table_num_entries =
	sizeof(clock_info_table) / sizeof(struct infineon_cat1_clock_info_t);
...
static cyhal_clock_t *_get_hal_obj_from_ord(uint32_t dt_ord)
{
	...
	for (uint32_t i = 0u; i < clock_info_table_num_entries; i++) {
