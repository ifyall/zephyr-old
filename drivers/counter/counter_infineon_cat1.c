...
static int counter_cat1_init(const struct device *dev)
{
	...
	/* Configure frequency */
	cyhal_timer_set_frequency(&data->counter_obj, config->counter_info.freq);
	if (rslt != CY_RSLT_SUCCESS) {
		return -EIO;
	}
				      ...
				      const struct counter_top_cfg *cfg)
{
	...
	} else if (cfg->ticks > data->counter_cfg.period) {
		err = -ETIME;
	...
	if ((err == 0) ||
	    ((err == -ETIME) && (cfg->flags & COUNTER_TOP_CFG_RESET_WHEN_LATE))) {
		/* Reconfigure timer */
		rslt = cyhal_timer_configure(&data->counter_obj, &data->counter_cfg);
		if (rslt != CY_RSLT_SUCCESS) {
			return -EIO;
