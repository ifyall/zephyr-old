...
cyhal_gpio_t cat1_pinctrl_get_cyhal_gpio(const struct pinctrl_dev_config *config, uint8_t index);
...
void master_event_handler(void *callback_arg, cyhal_i2c_event_t event)
{
	...
	if ((CYHAL_I2C_MASTER_ERR_EVENT & event) != 0U) {
		...
		cyhal_i2c_abort_async(&data->i2c_master_obj);
...
static int i2c_cat1_configure(const struct device *dev, uint32_t dev_config)
{
	...
	k_sem_take(&data->i2c_operation_sem, K_MSEC(I2C_CAT1_SEM_TIMEOUT_MS));
			     ...
			     uint8_t num_msgs, uint16_t addr)
{
	...
	for (uint32_t i = 0u; i < num_msgs; i++) {
		...
		k_sem_take(&data->i2c_transfer_sem, K_MSEC(I2C_CAT1_SEM_TIMEOUT_MS));
...
static int i2c_cat1_init(const struct device *dev)
{
	...
	k_sem_init(&data->i2c_transfer_sem, 0, 1);
	k_sem_init(&data->i2c_operation_sem, 0, 1);
