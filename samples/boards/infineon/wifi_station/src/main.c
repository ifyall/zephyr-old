...
extern whd_interface_t cyw43xxx_if;
...
void wifi_connect(void)
{
	...
	for (uint32_t conn_retries = 0; conn_retries < MAX_WIFI_RETRY_COUNT; conn_retries++) {
		ret = whd_wifi_join(cyw43xxx_if, &ssid, WHD_SECURITY_WPA2_AES_PSK,
				    CONFIG_CYW43XXX_WIFI_PASSWORD,
				    strlen(CONFIG_CYW43XXX_WIFI_PASSWORD));
		if (ret == CY_RSLT_SUCCESS) {
			ret = whd_wifi_get_ap_info(cyw43xxx_if, &bss_info, &security);
			if (ret != CY_RSLT_SUCCESS) {
...
void main(void)
{
	...
	PRINT_SCAN_TEMPLATE()
	ret = whd_wifi_scan(cyw43xxx_if, WHD_SCAN_TYPE_ACTIVE, WHD_BSS_TYPE_ANY,
